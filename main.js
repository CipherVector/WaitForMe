...
function join() {
	...
	server.on('login', (newProxyClient) => { // handle login
		if(config.whitelist && client.uuid !== newProxyClient.uuid) {
			newProxyClient.end("not whitelisted!\nYou need to use the same account as W4M or turn the whitelist off");
			return;
		}
...
function log(logmsg) {
	if (config.get("logging")) {
		fs.appendFile('Wait4Me.log', DateTime.local().toLocaleString({
			hour: '2-digit',
			minute: '2-digit',
			hour12: false
		}) + "	" + logmsg + "\n", err => {
...
function userInput(cmd, DiscordOrigin, discordMsg) {
	...
	switch (cmd) {
		case "start":
			...
			break;
		case "help":
			console.log("quit: quits the process, update: gives updatemessage, start: starts the queue progress, stops: the queue")
			brek;
		case "exit":
		case "quit":
